...
class __Mustache_a6d2d51cff38ca8e4aae627a434660e4 extends Mustache_Template
{
...
';
        $buffer .= $indent . '			<h2 class="change-bg-color">Related</h2>
';
        $buffer .= $indent . '			<ul class="related-projects-list">
';
        // 'related_project_posts' section
        $value = $context->find('related_project_posts');
        $buffer .= $this->section0c066e0f8392c7a690c8a28b01badd15($context, $indent, $value);
        $buffer .= $indent . '			</ul>
';
        $buffer .= $indent . '		</div>
';
...
';
    ...
    private function section0c066e0f8392c7a690c8a28b01badd15(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        if (!is_string($value) && is_callable($value)) {
            $source = '
					<li class="change-bg-color {{ class }}"><a href="{{ permalink }}">{{ post_title }}</a></li>
				';
            $result = call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
            ...
            foreach ($values as $value) {
                $context->push($value);
                $buffer .= $indent . '					<li class="change-bg-color ';
